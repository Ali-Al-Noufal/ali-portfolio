...
const AdminProjects = () => {
      ...
      useEffect(()=>{
        axios.get("https://ali-al-noufal.vercel.app/api/api/projects",{
          headers:{
      ...
      const editProject=(id)=>{
        ...
        axios.post(`https://ali-al-noufal.vercel.app/api/api/projects`,data,{
          headers:{
            "Accept":"application/json",
...
.catch(err=>console.log(err))
      ...
      const deleteProject=(id)=>{
          axios.delete(`https://ali-al-noufal.vercel.app/api/api/projects/${id}`,{
          headers:{
            "Accept":"application/json",
            'Content-Type':"application/json",
            Authorization:localStorage.getItem("token")
                ...
                return(
                    <div key={project.id}  className="h-80 w-full md:w-[30%] flex flex-col justify-between pointer-coarse ">
                        ...
                        <div className="h-27 w-full flex flex-col justify-between items-start relative">
                            ...
                            <p className="text-[#1D2130] font-normal text-[16px] dark:text-[#FFFFFF]">{project.lang}</p>
...
<button onClick={()=>deleteProject(project.id)} className="border w-20 bg-[#be2828] text-[#FFFFFF] h-10 rounded-sm">delete</button></div>
      ...
      <form className="w-[80vw] lg:w-200 h-350 flex flex-col items-center gap-10 bg-[#F1F4F9] my-10 border-2 rounded-3xl" onSubmit={handleForm}>
            ...
            <input type="text" placeholder="gh_url" className="bg-[#FFFFFF] border w-[60%] h-15 rounded-2xl px-3"
            onChange={(event)=>{setData({...data,["gh_url"]:event.target.value})}}  />
            <input type="text" placeholder="domain"  className="bg-[#FFFFFF] border w-[60%] h-15 rounded-2xl px-3"
            onChange={(event)=>{setData({...data,["domain"]:event.target.value})}}  />
            <input type="text" placeholder="basic_languages"  className="bg-[#FFFFFF] border w-[60%] h-15 rounded-2xl px-3"
            onChange={(event)=>{setData({...data,["basic_languages"]:event.target.value})}}  />
            <input type="text" placeholder="framework"  className="bg-[#FFFFFF] border w-[60%] h-15 rounded-2xl px-3"
            onChange={(event)=>{setData({...data,["framework"]:event.target.value})}}  />
            <input type="text" placeholder="libraries"  className="bg-[#FFFFFF] border w-[60%] h-15 rounded-2xl px-3"
            onChange={(event)=>{setData({...data,["libraries"]:event.target.value})}}  />
            <input type="text" placeholder="date"  className="bg-[#FFFFFF] border w-[60%] h-15 rounded-2xl px-3"
